...
/**
 * MA monogram from the brand kit:
...
export function MarkMA({ className = "", title = "Marco Antolini" }: MarkProps) {
	return (
		<svg
			...
			className={className}
		>
			{/* M — squared frame uprights with a center notch */}
			<path
			...
			<path
				d="M41 13.5 L41 23 L43.1 20.9 L44.6 24 L46 23.4 L44.5 20.4 L47.4 20.4 Z"
				fill="rgb(52 211 153)"
			/>
		</svg>
	);
...
export function WordmarkMA({ className = "" }: WordmarkProps) {
	return (
		<span className={`inline-flex items-center gap-2.5 ${className}`}>
			<MarkMA className="h-6 w-auto text-v2-ivory" />
			<span className="font-satoshi text-[15px] font-medium tracking-tight text-v2-ivory">
				Marco Antolini
			</span>
		</span>
	);
