		...
		<article className="mx-auto max-w-[1180px] px-5 pb-24 pt-28 sm:px-8 sm:pt-32">
						...
						<Reveal key={section.id} delay={index * 0.04} className={isWide ? "md:col-span-2" : undefined}>
							<section
								className={`h-full rounded-2xl border p-6 sm:p-8 ${
									isOutcomes ? `${accent.outcomesBorder} ${accent.outcomesBg}` : "site-surface-flat border-brand-border"
								}`}
								aria-labelledby={`${section.id}-heading`}
							>
								<p className="site-label text-brand-emerald">{sectionLabels[section.id] ?? section.title}</p>
			...
			<Reveal className="mt-14">
				<div className="site-frame flex flex-col items-start justify-between gap-6 rounded-2xl p-6 sm:flex-row sm:items-center sm:p-8">
					<div>
						<p className="site-label text-brand-emerald">{footer.eyebrow}</p>
