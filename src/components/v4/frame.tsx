...
export function BrowserFrame({ url, children, className = "", status }: BrowserFrameProps) {
	return (
		<div className={`v4-surface overflow-hidden rounded-2xl ${className}`}>
...
export function StatusDot({ tone = "emerald" }: { tone?: "emerald" | "amber" }) {
	...
	return (
		<span className="relative inline-flex h-2 w-2" aria-hidden="true">
			<span className={`v4-animate-pulse absolute inline-flex h-full w-full rounded-full ${color} opacity-60 motion-reduce:animate-none`} />
