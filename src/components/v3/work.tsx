...
export default function V3Work() {
	return (
		<section id="work" className="scroll-mt-24 border-t border-brand-border/60 py-20 sm:py-28">
			<Reveal>
				<p className="v3-label">Selected work</p>
				<h2 className="mt-4 max-w-2xl font-satoshi text-3xl font-bold tracking-tight text-brand-ivory sm:text-[2.6rem] sm:leading-[1.05]">
...
function FeaturedProject({ slug, flip }: { slug: string; flip: boolean }) {
	...
	return (
		<Reveal className="grid items-center gap-8 lg:grid-cols-2 lg:gap-14">
			{/* Surface */}
			<div className={`v3-corners ${flip ? "lg:order-2" : ""}`}>
				<BrowserFrame
			...
			<div className={flip ? "lg:order-1" : ""}>
				<h3 className="font-satoshi text-2xl font-bold tracking-tight text-brand-ivory sm:text-3xl">
					{project.title}
				</h3>
					...
					<dl className="mt-7 grid grid-cols-3 gap-px overflow-hidden rounded-xl border border-brand-border bg-brand-border/60">
						{study.highlights.map((h) => (
							<div key={h.label} className="bg-brand-zinc/40 px-3 py-4">
								<dt className="v3-label !text-[10px]">{h.label}</dt>
								<dd className="mt-2 font-satoshi text-[13px] font-medium leading-snug text-brand-ivory">
									{tidy(h.value)}
								</dd>
							</div>
						))}
